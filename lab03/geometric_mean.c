 ...
 */
DataChunk **monta_chunks(int m, int n, int **matrix, float *media_geometrica_colunas);
...
{
    #if DEBUG
        printf("Destruindo chunks\n");
    #endif
    for (int i = 0; i < THREAD_COUNT; i++)
    {
        #if DEBUG
            printf("Limpando chunk %d\n", i);
        #endif
        free(chunks[i]);
        #if DEBUG
            printf("Limpou chunk %d\n", i);
        #endif
    }
    free(chunks);
    #if DEBUG
        printf("Limpou chunks\n");
    #endif
}
...
void *calcula_media_geometrica(void *param)
{
    ...
    DataChunk *chunk = (DataChunk *)param; // type cast
...
#if DEBUG
        printf("Linha %d: Soma=%.2f, Media=%.2f\n", i, produtorio_coluna, media_coluna);
#endif
